...
func BuildSystemPrompt(opts BuildSystemPromptOptions) string {
	...
	b.WriteString("- Spawning multiple subagents that will run bash (parallel reviews, per-branch/per-PR audits, council/check-work workloads): create one sandbox per subagent (or per worktree) BEFORE spawning — a subagent cannot create its own — stage what each needs (worktree checked out, diff already written to a file, deps installed if the task calls for it), then pass sandboxIds so every child runs bash(sandboxId=...) with no approval gate. Skipping this means every gated command each parallel child runs queues its own approval prompt — multiplied by however many subagents are running at once.\n\n")
