...
- **Subagents run async** — one `subagent` tool, dispatched by `action`:
  `spawn` (default) starts a child and returns a job ID immediately; the
  main agent keeps working (or you keep chatting) while it runs in the
  background. `status` polls progress (one job or every job), `result`
  retrieves the final output once, `kill` stops one early (or every job at
  once) — see [docs/async-subagent-plan.md](docs/async-subagent-plan.md).
