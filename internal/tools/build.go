...
func BuildRegistry(opts BuildOptions) *Registry {
	...
	if opts.SandboxManager != nil {
		// action=create is parent-only, same reasoning as the subagent tool
		// below: a subagent may only use sandboxes its parent explicitly
		// authorized (docs/sandbox-plan.md), never mint its own. cp/destroy/
		// resurrect/list stay available to a child for now — whether a
		...
		// (docs/sandbox-plan.md "Subagents"), not decided here. list is
		// read-only regardless (browsing grants no access on its own — see
		// docs/sandbox-plan.md's "Crash recovery" section).
		reg.Register(NewSandboxTool(opts.Cwd, opts.SandboxManager, sandboxApproval, fileApproval, !opts.Child))
	}
...
func BindSubagentJobDone(reg *Registry, fn func(jobID, sessionID, toolCallID string, res ToolResult, killed bool)) {
...
// still-running job's own context (jobs otherwise have no time limit, see
// jobCtx's doc comment in Execute), so KillSubagents' explicit child-killing
// has a matching cancellation signal instead of leaving runJob's
// ctx-cancelled exit paths permanently dead code.
