...
// The command isn't secret-scrubbed here: it's the agent's own literal
// input, not tool output, and the one chokepoint that matters — tool
// results (read/bash-output/grep/...) — already scrubs anything a secret
// could enter the agent's context through. See guard.RedactSecrets and
// tools.TrimToolResult.
