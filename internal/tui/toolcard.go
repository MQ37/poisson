...
func formatToolCollapsed(b *Block, width int) string {
	...
	title := titleCaseTool(name) + providerTagFromInput(b.meta.ToolInput) + sandboxTagFromInput(b.meta.ToolInput)
...
func batchExpandedCallLines(input []byte, inner int) []string {
	...
	for i, c := range calls {
		...
		name := tools.CanonicalToolName(c.Tool) + providerTagFromInput(c.Input) + sandboxTagFromInput(c.Input)
