...
func TestBuildRegistry_ParentWithStore(t *testing.T) {
	...
	for _, w := range []string{"recall", "list_sessions", "read_messages", "set_title", "subagent"} {
		if _, ok := reg.Get(w); !ok {
...
func TestBuildRegistry_Child(t *testing.T) {
	...
	for _, w := range []string{"read", "write", "edit", "bash", "batch", "grep", "glob", "web_ask", "web_search", "recall", "list_sessions", "read_messages", "set_title"} {
		if _, ok := reg.Get(w); !ok {
