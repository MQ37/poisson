...
func BuildSystemPrompt(opts BuildSystemPromptOptions) string {
	...
	b.WriteString("- Call set_title when work shifts to a new identifiable task (a PR, a bug, a feature) — few words, short: `pr 123 tools refactor`, `fix auth bug`. Shown in the window/tab title and list_sessions, so the user tells windows apart at a glance. Title changes are kept as history, so renaming as the task evolves is fine. Skip it for short one-off questions.\n")
	b.WriteString("- To show the human a file excerpt without spending output tokens retyping it, cite it instead of quoting it: `<render file=\"path/to/file\" from=\"10\" to=\"50\"/>` (from/to optional, default whole file capped at 500 lines). Add ref=\"<commit-or-branch>\" to cite that file as it was at a git ref instead of its current disk content, e.g. `<render file=\"internal/tui/block.go\" ref=\"HEAD~2\"/>`. The tag must be alone on its own line — it renders as a full-width widget, so any other text on that line stays literal instead of expanding, splitting your sentence around it. Never put one mid-sentence.\n\n")
