...
func BuildSystemPrompt(opts BuildSystemPromptOptions) string {
	...
	b.WriteString("- sandbox(action=create) once, then bash(sandboxId=...) runs with no approval gate at all — the container is the safety boundary. No default workspace: pass hostPath to bind-mount a directory, or omit it for an isolated container. read/write/edit/grep/glob need no sandboxId — they always target the host path directly. Name the sandbox descriptively; check sandbox(action=list) first and reuse one you recognize instead of duplicating it. sandbox(action=destroy) only kills the container, never hostPath.\n")
	b.WriteString("- Don't destroy a sandbox just because the current step is done — a follow-up prompt, a verifier subagent, or the next task in this session can likely reuse it (toolchains/deps already installed there). Keep it running until the user is clearly finished with that line of work or asks to clean up; destroying and recreating for every step re-pays setup cost for no benefit.\n")
	...
	b.WriteString("- Before calling any non-trivial code change done, re-scan what you just wrote yourself, don't wait for the user to ask for a cleanup pass: every new function/type has 2+ real callers or gets inlined right now; no single-letter variable names outside a loop index; no defensive/streaming/retry machinery sized for a scale or failure mode the task doesn't actually establish as real yet.\n")
	...
	b.WriteString("- A spawned subagent runs in the background; don't sleep/poll waiting for it. Its completion is pushed to you automatically the moment it finishes — do other work in the meantime, or just end your turn if there's none, rather than looping on subagent(action=status) or a bash sleep.\n")
	b.WriteString("- To show the human a file excerpt without spending output tokens retyping it, cite it instead of quoting it: `<render file=\"path/to/file\" from=\"10\" to=\"50\"/>` (from/to optional, default whole file capped at 500 lines; omitting both is fine, never invert them — from must be <= to or the citation errors). file resolves relative to session cwd, exactly like read/write/edit — not relative to a bash call's own workdir override, so use the full path when citing a file outside session cwd (e.g. a cloned repo).\n")
	b.WriteString("  Add ref=\"<commit-or-branch>\" to cite that file as it was at a git ref instead of its current disk content, e.g. `<render file=\"internal/tui/block.go\" ref=\"HEAD~2\"/>`. With ref, file is resolved inside the repo containing it, not against session cwd directly — if cwd is a workspace/index directory sitting above several repos (common multi-project layout) and the bare path doesn't exist right under cwd, it is searched one or two directories down; a name that matches more than one repo there (e.g. a bare \"README.md\" when several sibling repos each have one) is rejected as ambiguous instead of guessing. Always give enough of the path to be unambiguous on first try — repo-qualify it, e.g. `<render file=\"myrepo/internal/foo.go\" ref=\"HEAD~2\"/>`, not a bare filename — rather than finding out from an error.\n")
	b.WriteString("  The tag must be alone on its own line — it renders as a full-width widget, so any other text on that line stays literal instead of expanding, splitting your sentence around it. Never put one mid-sentence.\n\n")
	b.WriteString("- Handing back a file meant to be opened rather than read as text (an HTML report, a generated image, anything not already visible in a diff) — give it as a `file://<absolute-path>` link, bare or as markdown `[label](file://...)`, never a bare path. The TUI turns a `file://`/`http(s)://` link into a real clickable terminal hyperlink; a bare path is just text the human has to go open by hand. See the `canvas` skill for building an HTML report worth linking this way.\n\n")
